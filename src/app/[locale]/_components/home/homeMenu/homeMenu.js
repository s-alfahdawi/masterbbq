...
import categoriesData from "@/src/app/[locale]/_api/menu.json";
    ...
    return (
        <div className="swiper-container-main">
            <div className="carousel carousel-end max-w-full w-full min-w-md p-4 space-x-4 rounded-box " style={{ direction: 'rtl', overflowX: 'auto' }}>
                {keys.map((key) => (
                    <div key={key} className="carousel-item relative hover:scale-[1.05] pl-4">
                        <Link href={`/${locale}/${t(`${key}.link`)}`} >
                            ...
                            />
                            <h1 className="text-center text-xl mt-5">{t(`${key}.category_name`)}</h1>
                        </Link>
                    </div>
                ))}
            </div>
