...
import galleryItems from "../../../_api/photos.json";
...
const Carousel = () => {
  ...
  return (
    <main className="py-8 mb-9 overflow-clip">
      <motion.div className="flex" style={{ x: xTranslation }} ref={ref}>
        {photos.map((item, idx) => (
          <Card image={item.link} key={item.id} onClick={handleCardClick} />
        ))}
      </motion.div>
      ...
      {isModalOpen && (
        ...
        >
          <motion.div
            className="max-w-md w-full p-4 bg-base-100 rounded-lg"
